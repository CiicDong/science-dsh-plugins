...
# Each preset is a directory that MUST hold agent.cordis.yml at its root.
# Copy the leaf folders, not preset\preset (that parent occupies the id
# without a composition file and the roster reports it as broken).
foreach ($id in @('router-standard', 'router-spec')) {
  $source = Join-Path $root "preset\preset\$id"
  $target = Join-Path $presetRoot $id
  if (-not (Test-Path $source)) {
    Write-Host "跳过 $id：源目录不存在 $source" -ForegroundColor Yellow
    continue
  }
  if (Test-Path $target) {
...
Write-Host '2. GUI 新建会话 → 选择 Router Standard (experimental) 或 Router Spec (experimental)' -ForegroundColor Yellow
